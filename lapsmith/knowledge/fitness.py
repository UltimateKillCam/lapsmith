...
# richer channels (small, corroborating weights; 0 contribution when not live)
W_CLEANSPIN = 0.4               # less per-wheel wheelspin for the same drive = cleaner hookup
W_BODYROLL = 0.4                # less left-right suspension asymmetry = better body control
W_RIDE = 0.2                    # lower vertical-g harshness = smoother ride
...
class LapTelemetry:
    ...
    # richer per-bin channels (0 when the source channel isn't live -> no composite effect)
    wheelspin: List[float] = field(default_factory=list)  # worst wheel positive slip (throttle on)
    vert_g: List[float] = field(default_factory=list)     # mean |vertical g| (ride harshness)
    roll: List[float] = field(default_factory=list)       # L-R suspension asymmetry (body roll)
    n_frames: int = 0
...
def bin_lap(packets: List[Packet], n_bins: int = TELEM_BINS) -> LapTelemetry:
    ...
    lt = LapTelemetry(grip=[0.0] * n_bins, exit_g=[0.0] * n_bins, slip=[0.0] * n_bins,
                      ...
                      steer_in=[0.0] * n_bins, wheelspin=[0.0] * n_bins,
                      vert_g=[0.0] * n_bins, roll=[0.0] * n_bins)
    ...
    for p, x in zip(pkts, pos):
        ...
        vert_acc[b] += abs(p.accel_y) / 9.80665            # vertical g (ride harshness)
        roll_acc[b] += (abs(p.susp_norm_fl - p.susp_norm_fr)
                        + abs(p.susp_norm_rl - p.susp_norm_rr)) / 2.0   # L-R body roll
        ...
        if p.accel >= THROTTLE_ON:
            ...
            spin_acc[b] += max(0.0, p.tire_slip_ratio_fl, p.tire_slip_ratio_fr,
                               p.tire_slip_ratio_rl, p.tire_slip_ratio_rr)   # worst wheelspin
            if abs(fwd) > 0.05:
                any_exit = True
    for b in range(n_bins):
        if cnt[b]:
            ...
            lt.vert_g[b] = vert_acc[b] / cnt[b]
            lt.roll[b] = roll_acc[b] / cnt[b]
        if thr_cnt[b]:
            lt.exit_g[b] = exit_acc[b] / thr_cnt[b]
            lt.slip[b] = slip_acc[b] / thr_cnt[b]
            lt.wheelspin[b] = spin_acc[b] / thr_cnt[b]
...
class CompositeResult:
    ...
    traction: float = 0.0
    minspeed: float = 0.0
    cleanspin: float = 0.0       # richer: less wheelspin per-wheel
    bodyroll: float = 0.0        # richer: less body roll asymmetry
    ride: float = 0.0            # richer: less vertical harshness
...
def composite(cand: LapTelemetry, ref: LapTelemetry, discipline: str,
    ...
    for b in range(n):
        if ref.corner[b] or cand.corner[b]:
            ...
            roll_d += ref.roll[b] - cand.roll[b]      # less L-R asymmetry = better
            roll_n += 1
        if ref.onthrottle[b] or cand.onthrottle[b]:
            ...
            spin_d += ref.wheelspin[b] - cand.wheelspin[b]   # less wheelspin = better
            exit_n += 1
        if ref.vert_g[b] > 0.0 or cand.vert_g[b] > 0.0:
            ride_d += ref.vert_g[b] - cand.vert_g[b]   # less vertical harshness = better
            ride_n += 1
    ...
    res.cleanspin = spin_d / exit_n if exit_n else 0.0
    res.bodyroll = roll_d / roll_n if roll_n else 0.0
    res.ride = ride_d / ride_n if ride_n else 0.0
    ...
    res.delta = (W_GRIP * res.grip + W_EXIT * res.exit
                 ...
                 + W_CLEANSPIN * res.cleanspin + W_BODYROLL * res.bodyroll
                 + W_RIDE * res.ride)
